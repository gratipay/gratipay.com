...
class TestMarkdown(Harness):
    ...
    def test_r_renders_https_links(self):
    ...
    def test_r_escapes_javascript_links(self):
    ...
    def test_r_doesnt_allow_any_explicit_anchors(self):
        expected = '<p>foo</p>\n'
    ...
    def test_ras_scrubs_comments(self):
        # http://stackoverflow.com/a/19730306
        assert markdown.render_and_scrub('<img<!-- --> src=x onerror=alert(1);//><!-- -->') == \
                                              'src=x onerror=alert(1);//&gt;&lt;!\u2013 \u2013&gt;'

    def test_ras_renders_entity_references(self):
        assert markdown.render_and_scrub('&trade;') == '\u2122'

    def test_r_does_not_render_entity_references_it_really_is_striptags(self):
        assert markdown.render('&trade;') == '<p>&trade;</p>\n'
        assert markdown.render_and_scrub('&trade;') == '\u2122'

    def test_ras_does_not_double_escape(self):
        assert markdown.render_and_scrub('&amp;') == '&amp;'

    def test_ras_returns_a_Markup(self):
        assert type(markdown.render_and_scrub('<p>&amp;</p>')) is markdown.Markup

    def test_ras_escapes_single_quotes_but_still_doesnt_double_escape(self):
